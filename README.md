...
- **👨‍👩‍👧 Relationships** — Typed links between people (`partner`, `parent`/`child`, `sibling`, `custom` with optional free-text label overriding the type name). Stored canonically directed (`parent` always parent→child) with read-time inverse derivation; symmetric types share a single row. Guards reject self-links and impossible mutual parent loops.
