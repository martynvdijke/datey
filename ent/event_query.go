...
func (_q *EventQuery) QueryContact() *ContactQuery {
...
func (_q *EventQuery) QueryNotificationLogs() *NotificationLogQuery {
...
func (_q *EventQuery) WithContact(opts ...func(*ContactQuery)) *EventQuery {
	query := (&ContactClient{config: _q.config}).Query()
...
func (_q *EventQuery) WithNotificationLogs(opts ...func(*NotificationLogQuery)) *EventQuery {
	query := (&NotificationLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if query := _q.withNotificationLogs; query != nil {
		if err := _q.loadNotificationLogs(ctx, query, nodes,
			func(n *Event) { n.Edges.NotificationLogs = []*NotificationLog{} },
			func(n *Event, e *NotificationLog) { n.Edges.NotificationLogs = append(n.Edges.NotificationLogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EventQuery) loadContact(ctx context.Context, query *ContactQuery, nodes []*Event, init func(*Event), assign func(*Event, *Contact)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		if nodes[i].contact_events == nil {
			continue
		}
		fk := *nodes[i].contact_events
...
func (_q *EventQuery) loadNotificationLogs(ctx context.Context, query *NotificationLogQuery, nodes []*Event, init func(*Event), assign func(*Event, *NotificationLog)) error {
	...
	query.Where(predicate.NotificationLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(event.NotificationLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.event_notification_logs
		if fk == nil {
			return fmt.Errorf(`foreign-key "event_notification_logs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "event_notification_logs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
