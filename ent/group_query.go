...
func (_q *GroupQuery) QueryPeople() *PersonQuery {
...
func (_q *GroupQuery) WithPeople(opts ...func(*PersonQuery)) *GroupQuery {
	query := (&PersonClient{config: _q.config}).Query()
...
func (_q *GroupQuery) loadPeople(ctx context.Context, query *PersonQuery, nodes []*Group, init func(*Group), assign func(*Group, *Person)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.PeopleTable)
		s.Join(joinT).On(s.C(person.FieldID), joinT.C(group.PeoplePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.PeoplePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.PeoplePrimaryKey[1]))
