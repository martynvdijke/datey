...
func (_q *PersonQuery) QueryEvents() *EventQuery {
...
func (_q *PersonQuery) QueryGroups() *GroupQuery {
...
func (_q *PersonQuery) WithEvents(opts ...func(*EventQuery)) *PersonQuery {
	query := (&EventClient{config: _q.config}).Query()
...
func (_q *PersonQuery) WithGroups(opts ...func(*GroupQuery)) *PersonQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PersonQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Person, error) {
	...
	if query := _q.withEvents; query != nil {
		if err := _q.loadEvents(ctx, query, nodes,
			func(n *Person) { n.Edges.Events = []*Event{} },
			func(n *Person, e *Event) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *PersonQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Person, init func(*Person), assign func(*Person, *Event)) error {
	...
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(person.EventsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.person_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "person_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "person_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *PersonQuery) loadGroups(ctx context.Context, query *GroupQuery, nodes []*Person, init func(*Person), assign func(*Person, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(person.GroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(person.GroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(person.GroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(person.GroupsPrimaryKey[0]))
