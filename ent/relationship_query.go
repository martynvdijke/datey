...
type RelationshipQuery struct {
	...
	withFromPerson *PersonQuery
	withToPerson   *PersonQuery
...
// QueryFromPerson chains the current query on the "from_person" edge.
func (_q *RelationshipQuery) QueryFromPerson() *PersonQuery {
...
// QueryToPerson chains the current query on the "to_person" edge.
func (_q *RelationshipQuery) QueryToPerson() *PersonQuery {
	query := (&PersonClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *RelationshipQuery) Clone() *RelationshipQuery {
	...
	return &RelationshipQuery{
		...
		withFromPerson: _q.withFromPerson.Clone(),
		withToPerson:   _q.withToPerson.Clone(),
...
// WithFromPerson tells the query-builder to eager-load the nodes that are connected to
// the "from_person" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RelationshipQuery) WithFromPerson(opts ...func(*PersonQuery)) *RelationshipQuery {
	query := (&PersonClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withFromPerson = query
	return _q
}

// WithToPerson tells the query-builder to eager-load the nodes that are connected to
// the "to_person" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RelationshipQuery) WithToPerson(opts ...func(*PersonQuery)) *RelationshipQuery {
	query := (&PersonClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withToPerson = query
	return _q
}
...
//		GroupBy(relationship.FieldFromID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Relationship.Query().
//		Select(relationship.FieldFromID).
//		Scan(ctx, &v)
...
func (_q *RelationshipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Relationship, error) {
	...
	if query := _q.withFromPerson; query != nil {
		if err := _q.loadFromPerson(ctx, query, nodes, nil,
			func(n *Relationship, e *Person) { n.Edges.FromPerson = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withToPerson; query != nil {
		if err := _q.loadToPerson(ctx, query, nodes, nil,
			func(n *Relationship, e *Person) { n.Edges.ToPerson = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RelationshipQuery) loadFromPerson(ctx context.Context, query *PersonQuery, nodes []*Relationship, init func(*Relationship), assign func(*Relationship, *Person)) error {
...
func (_q *RelationshipQuery) loadToPerson(ctx context.Context, query *PersonQuery, nodes []*Relationship, init func(*Relationship), assign func(*Relationship, *Person)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Relationship)
	for i := range nodes {
		fk := nodes[i].ToID
...
func (_q *RelationshipQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withFromPerson != nil {
			_spec.Node.AddColumnOnce(relationship.FieldFromID)
		}
		if _q.withToPerson != nil {
			_spec.Node.AddColumnOnce(relationship.FieldToID)
		}
	}
