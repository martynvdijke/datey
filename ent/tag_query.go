...
func (_q *TagQuery) QueryPeople() *PersonQuery {
...
func (_q *TagQuery) WithPeople(opts ...func(*PersonQuery)) *TagQuery {
	query := (&PersonClient{config: _q.config}).Query()
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withPeople; query != nil {
		if err := _q.loadPeople(ctx, query, nodes,
			func(n *Tag) { n.Edges.People = []*Person{} },
			func(n *Tag, e *Person) { n.Edges.People = append(n.Edges.People, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadPeople(ctx context.Context, query *PersonQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Person)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.PeopleTable)
		s.Join(joinT).On(s.C(person.FieldID), joinT.C(tag.PeoplePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.PeoplePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.PeoplePrimaryKey[1]))
