...
func (_u *TagUpdate) AddPersonIDs(ids ...int) *TagUpdate {
	_u.mutation.AddPersonIDs(ids...)
	return _u
}

// AddPeople adds the "people" edges to the Person entity.
func (_u *TagUpdate) AddPeople(v ...*Person) *TagUpdate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
func (_u *TagUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.PeopleCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.PeopleTable,
			Columns: tag.PeoplePrimaryKey,
			Bidi:    false,
...
// AddPersonIDs adds the "people" edge to the Person entity by IDs.
func (_u *TagUpdateOne) AddPersonIDs(ids ...int) *TagUpdateOne {
	_u.mutation.AddPersonIDs(ids...)
	return _u
}

// AddPeople adds the "people" edges to the Person entity.
func (_u *TagUpdateOne) AddPeople(v ...*Person) *TagUpdateOne {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
// ClearPeople clears all "people" edges to the Person entity.
func (_u *TagUpdateOne) ClearPeople() *TagUpdateOne {
	_u.mutation.ClearPeople()
	return _u
}

// RemovePersonIDs removes the "people" edge to Person entities by IDs.
func (_u *TagUpdateOne) RemovePersonIDs(ids ...int) *TagUpdateOne {
	_u.mutation.RemovePersonIDs(ids...)
	return _u
}

// RemovePeople removes "people" edges to Person entities.
func (_u *TagUpdateOne) RemovePeople(v ...*Person) *TagUpdateOne {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
